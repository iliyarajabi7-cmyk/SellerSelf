...
config_seller.ensure_config(_db)
save_db(_db)
...
async def check_fjoin_status(user_id, db):
    ...
    for ch in db["config"]["fjoin"]:
        try:
            ...
            if member.status.value in ['left', 'kicked', 'banned']: raise Exception
...
async def message_handler(message: types.Message):
    try:
        ...
        if user_id == ADMIN_ID:
            ...
            if txt == "/panelset":
                config_seller.ensure_config(db); save_db(db)
                user_states[user_id] = config_seller.admin_start_state()
                return await message.answer(config_seller._ADMIN_STEPS[0][1], reply_markup=cancel_keyboard())
        ...
        if state in ["wait_mah_amount", "wait_bulk_mah_amount"]:
            ...
            try:
                ...
                await message.answer(f"❌ خطا در ارسال ��د: {e}", reply_markup=cancel_keyboard())
            ...
            kb_admin = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ تایید نمایندگی", callback_data=f"appr_resell_full_{user_id}", style=ButtonStyle.SUCCESS)],
                [InlineKeyboardButton(text="❌ رد درخواست", callback_data=f"reject_resell_full_{user_id}", style=ButtonStyle.DANGER)]
            ])
...
async def query_handler(callback_query: types.CallbackQuery):
    try:
        ...
        if data == "cancel_action": 
            ...
            if is_disc:
                kb = [
                    [InlineKeyboardButton(text="📦 خرید عمده با تخفیف نمایندگی", callback_data="menu_buybulk_mah", style=ButtonStyle.SUCCESS)],
            ...
            await callback_query.message.edit_text("🛒 **بازارچه خرید نمایندگی**\n\nلیست پن��‌های موجود برای خرید:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
            ...
            if trans_c:
                for u in db:
                    if u != "config" and u != "reseller_market" and u.isdigit():
                        if db[u].get("reseller_owner") == int(seller_id):
            ...
            if saved_phone:
                await callback_query.message.edit_text(f"⏳ در حال ارتباط با سرورهای تلگرام...")
