...
def calculate_remaining_time(user_id):
    ...
    if is_active:
        text += "وضعیت فعلی سلف: روشن\nدر صورت اضافه کردن قابلیت جدید، هزینه ۱ ساعت آن در لحظه کسر می‌شود.\n\n"
    else:
    ...
    if has_full: text += f"پکیج فول VIP — مصرف: `{prices.get('full_package', 50)}` میلی‌آمپر\n"
        ...
        for m in active_modules:
            cost = prices.get(m, 0)
            if cost > 0: text += f"{names.get(m, m)} — مصرف: `{cost}` میلی‌آمپر\n"
            
    text += f"\nوضعیت مجموع مصرف این پکیج:\n`{drain}` میلی‌آمپر (معادل `{cost_toman:,}` تومان در ساعت)\n"
    return text
...
def main_menu_keyboard(user_id):
    kb = [
        ...
        [InlineKeyboardButton(text="پشتیبانی", callback_data="support_menu", style=ButtonStyle.DANGER), 
         InlineKeyboardButton(text="چنل", url="https://t.me/Im_Iliiya", style=ButtonStyle.DANGER)],
...
def app_store_keyboard(user_id):
    ...
    fp_text = "فعال: پکیج فول VIP" if has_full else f"غیرفعال: پکیج فول VIP ({prices.get('full_package', 50)} mAh)"
    ...
    for row in layout:
        ...
        for key in row:
            ...
            icon = "[فعال]" if is_on else "[غیرفعال]"
